...
const MyNotes = () => {
  ...
  // Filter notes that are either created by the user or assigned to the user
  const myNotes = notes.filter(note => 
    note.userId === user?.id || note.assignedTo === user?.email
  );

  const filteredNotes = myNotes.filter(note =>
    (note.text || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
    (note.category || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
    (note.assignedTo || '').toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-gray-600 mt-2">Notes created by you and assigned to you</p>
            </div>
